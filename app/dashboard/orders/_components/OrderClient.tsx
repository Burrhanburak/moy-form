...
export default function OrderClient({ orders: initialOrders }: OrderClientProps) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status.toLowerCase()) {
      ...
      case 'completed':
        return {
          text: 'Paid',
          className: 'bg-emerald-100 text-emerald-500 dark:bg-emerald-950 dark:text-emerald-500'
        };
      case 'processing':
        return {
          text: 'Processing',
          className: 'bg-blue-100 text-blue-500 dark:bg-blue-950 dark:text-blue-500'
        };
      case 'pending_payment':
      case 'pending':
        return {
          text: 'Pending',
          className: 'bg-yellow-100 text-yellow-500 dark:bg-yellow-950 dark:text-yellow-500'
        };
      case 'canceled':
        return {
          text: 'Canceled',
          className: 'bg-red-100 text-red-500 dark:bg-red-950 dark:text-red-500'
        };
      case 'failed':
        return {
          text: 'Failed',
      ...
      default:
        return {
          text: status,
          className: 'bg-gray-100 text-gray-500 dark:bg-gray-950 dark:text-gray-500'
        };
    }
  };
  ...
  const filteredOrders = orders.filter(order => {
    ...
    if (statusFilter === 'paid') {
      return order.status.toLowerCase() === 'paid' || order.status.toLowerCase() === 'completed';
    }
                ...
                <div className="grid grid-cols-1 gap-4 md:gap-6 md:grid-cols-2 lg:grid-cols-3">
                  <div className="rounded-2xl bg-white p-4 md:p-6 text-black dark:bg-[#171719] dark:text-white border border-gray-200 dark:border-[#313131]">
                    <div className="space-y-1.5 pb-2">
                      <span className="text-black dark:text-white text-sm md:text-base">Total Orders</span>
                    </div>
                    <div className="pt-0">
                  ...
                  <div className="rounded-2xl bg-white p-4 md:p-6 text-black dark:bg-[#171719] dark:text-white border border-gray-200 dark:border-[#313131]">
                    <div className="space-y-1.5 pb-2">
                      <span className="text-black dark:text-white text-sm md:text-base">Today&apos;s Revenue</span>
                    </div>
                    <div className="pt-0">
                      <h3 className="text-xl md:text-2xl">{formatPrice(todayRevenue)}</h3>
                    </div>
                  </div>
  
                  <div className="rounded-2xl bg-white p-4 md:p-6 text-black dark:bg-[#171719] dark:text-white border border-gray-200 dark:border-[#313131]">
                    <div className="space-y-1.5 pb-2">
                      <span className="text-black dark:text-white text-sm md:text-base">Total Revenue</span>
                    </div>
                    <div className="pt-0">
                      <h3 className="text-xl md:text-2xl">{formatPrice(cumulativeRevenue)}</h3>
                    </div>
                  </div>
                </div>
                      ...
                      <table className="w-full text-sm table-fixed caption-bottom">
                        <thead>
                          <tr className="border-b border-gray-200 dark:border-[#313131] bg-white dark:bg-[#171719]">
                            <th className="h-12 px-4 text-left align-middle font-medium" style={{ width: 150 }}>Customer</th>
                            <th className="h-12 px-4 text-left align-middle font-medium" style={{ width: 150 }}>Amount</th>
                            <th className="h-12 px-4 text-left align-middle font-medium" style={{ width: 150 }}>Product</th>
                            <th className="h-12 px-4 text-left align-middle font-medium" style={{ width: 150 }}>Status</th>
                            <th className="h-12 px-4 text-left align-middle font-medium hidden md:table-cell" style={{ width: 150 }}>Invoice number</th>
                            <th className="h-12 px-4 text-left align-middle font-medium hidden md:table-cell" style={{ width: 150 }}>
                              <div className="flex items-center">Date
                                <IconChevronDown className="h-4 w-4 opacity-50 text-black dark:text-white" />
                              </div>
                            </th>
                            <th className="h-12 px-4 text-left align-middle font-medium" style={{ width: 120 }}>Actions</th>
                          </tr>
                        </thead>
  ...
  {filteredOrders.length === 0 ? (
    ...
    filteredOrders.map((order) => (
      <tr key={order.id} className="hover:bg-muted/50 data-[state=selected]:bg-muted border-b transition-colors">
        <td className="p-2 md:p-4 align-middle">
          <div className="flex items-center gap-2 md:gap-3">
            <div className="dark:bg-polar-900 dark:border-polar-700 z-2 relative flex shrink-0 items-center justify-center rounded-full border-2 border-gray-200 bg-gray-50 text-xs md:text-sm h-6 w-6 md:h-8 md:w-8">
        ...
        <td className="p-2 md:p-4 align-middle text-xs md:text-sm">{formatPrice(order.totalPrice)}</td>
        <td className="p-2 md:p-4 align-middle">
          <div className="flex flex-col">
            <span className="font-medium text-xs md:text-sm">{order.packageName}</span>
        ...
        <td className="p-2 md:p-4 align-middle text-xs md:text-sm hidden md:table-cell">{formatDate(order.createdAt)}</td>
