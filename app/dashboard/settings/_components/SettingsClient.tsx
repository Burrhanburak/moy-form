...
import { getUserProfile, updateUserProfile, changePassword, DeleteUser } from "@/app/action/user-action";
...
function SubmitButton({ text = "Save", isPending }: { text?: string; isPending?: boolean }) {
  return (
    <button
      type="submit"
      disabled={isPending}
...
export default function Page() {
  ...
  const [allowMultipleSubscriptions, setAllowMultipleSubscriptions] = useState(false);
  const [allowPriceChanges, setAllowPriceChanges] = useState(true);
  const [prorationSetting, setProrationSetting] = useState("prorate");
  const [newOrderNotifications, setNewOrderNotifications] = useState(true);

  const [state, formAction, isPending] = useActionState(updateUserProfile, { error: undefined, success: false, message: "" });
  const [passwordState, passwordFormAction, isPendingPassword] = useActionState(changePassword, { error: undefined, success: false, message: "" });
  const [deleteState, deleteFormAction, isPendingDelete] = useActionState(DeleteUser, { error: undefined, success: false, message: "" });
  ...
  useEffect(() => {
    async function fetchUser() {
      ...
      setUser(userData);
      // Parse existing social media links if any
      if (userData?.socialMedia) {
        try {
          const links = JSON.parse(userData.socialMedia);
          setSocialMediaLinks(Array.isArray(links) ? links : [userData.socialMedia]);
        } catch {
          setSocialMediaLinks([userData.socialMedia]);
        }
      }
      setLoading(false);
    }
...
//   if (loading) return <div className="flex flex-1 flex-col items-center justify-center dark:bg-[#171719]"><Loader className="w-8 h-8 animate-spin text-gray-500" /></div>;
      ...
      <div className="dark:md:bg-[#171719] dark:border-[#313131] dark:border-solid shadow-[inset_0_1px_2px_0_rgba(255,255,255,0.25)] md:shadow-xs relative flex w-full flex-col items-center rounded-2xl px-4 md:overflow-y-auto md:border md:bg-white md:px-8">
        ...
        <div className="flex h-full w-full flex-col max-w-(--breakpoint-sm)! max-w-(--breakpoint-xl)">
          ...
          <div className="flex w-full flex-col gap-y-4 py-8 md:flex-row md:items-center md:justify-between md:py-8">
            <h4 className="whitespace-nowrap text-2xl font-medium dark:text-white">Settings</h4>
          </div>
                  ...
                  <div className="dark:ring-0 dark:bg-[#171719] dark:border-[#313131] dark:border-solid shadow-[inset_0_1px_2px_0_rgba(255,255,255,0.25)] w-full flex-col overflow-hidden border rounded-2xl bg-transparent ring-1 ring-gray-200 dark:ring-0">
                    ...
                    <div className="flex gap-x-12 gap-y-4 p-4 flex-col md:flex-row md:items-start md:justify-between">
                      <div className="md:max-w-1/2 flex w-full flex-col">
                        ...
                        <p className="dark:text-polar-500 text-xs text-gray-500">Unique identifier for your account</p>
                      </div>
                    ...
                    <div className="flex gap-x-12 gap-y-4 p-4 flex-col md:flex-row md:items-start md:justify-between">
                      <div className="md:max-w-1/2 flex w-full flex-col">
                        ...
                        <p className="dark:text-polar-500 text-xs text-gray-500">Account creation date</p>
                      </div>
                      <div className="flex w-full flex-row gap-y-2 md:w-full md:justify-end">
                    ...
                    <div className="flex flex-col gap-y-4 p-4">
                      <div className="space-y-8">
                        <div className="space-y-6 grid grid-cols-1 gap-6 sm:grid-cols-12">
                          ...
                          <div className="sm:col-span-2">
                            ...
                            <div className="relative cursor-pointer group">
                              <img
                                alt={user?.name || 'User'}
                                className="aspect-square h-16 w-16 rounded-full object-cover bg-gray-200"
                                src={`${
                                  user?.image ||
                        ...
                        {/* Social Media */}
                        <div>
                          <label className="mb-2 block text-sm font-medium">Social Media Links</label>
                            ...
                            {socialMediaLinks.length > 0 && (
                              <div className="space-y-2">
                                {socialMediaLinks.map((link, index) => (
                                  <div key={index} className="flex items-center gap-2 p-2 bg-gray-50 dark:bg-[#313131] rounded-lg">
                ...
                <div className="flex flex-row">
                  <div className="flex flex-col gap-y-1">
                    ...
                    <p className="text-sm text-gray-500 dark:text-polar-400">
                      Expires on October 29, 2025 — Last used on September 29, 2025
                    </p>
                  </div>
                </div>
                <div className="flex flex-row items-center gap-2 text-gray-500 dark:text-polar-400">
                  ...
                  >
                    <div className="flex flex-row items-center">
                        {isPendingDelete ? <Loader className="w-4 h-4 animate-spin" /> : <div className="flex flex-row items-center">Delete</div>}
                    </div>
                  </button>
                    </DialogFooter>
                   </Dialog>
  ...
  <Dialog open={isPasswordDialogOpen} onOpenChange={setIsPasswordDialogOpen}>
    <DialogContent className="sm:max-w-md">
      ...
      <form id="change-password-form" action={passwordFormAction} className="space-y-4">
        {/* Current Password */}
        <div>
        ...
        <DialogFooter className="flex gap-2">
          ...
          <SubmitButton text="Change Password" isPending={isPendingPassword} />
        </DialogFooter>
      </form>
    </DialogContent>
  </Dialog>
